...
import StoryblockClient from 'storyblok-js-client'
const Storyblok = new StoryblockClient({accessToken:'T8mLujNezqZvQjHW6yF0Xwtt&cv=1629107381'})
export default {
  name: "Home",
  components: {
    Hero,
  ...
  mounted() {
    ...
    axios
      .get( Storyblok.get('cdn/stories/landing-page', {version:"draft"}))
      .then(response => (this.data = Storyblok.get('cdn/stories/landing-page', {version:"draft"}))).then(() =>{
        gsap.from(".home-bg",{autoAlpha:0,duration:1,rotate:8,delay:1, x:20})
      })
