<template>
    ...
    <div class="hero-container">
      <div class="left-side">
        ...
        <div class="btn-group" >
          <router-link to="/AboutPage" :class="hideBtn">{{buttonText}}</router-link>
          <router-link to="/ServicesPage" :class="hideBtn2">Služby</router-link>
        </div>
      </div>
    </div>
</template>
...
export default {
  ...
  mounted(){
    const { title, subtext, bg} = this.$refs

    const t1 = gsap.timeline({ defaults: { duration: .7 }})

    t1.from(bg, {opacity:0, ease: "power2.out"})
      .from(title, {opacity:0, y:-10, ease: "power2.out"})
      .from(subtext, {opacity:0}, "-=0.2")
